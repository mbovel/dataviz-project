...
class GraphSimulation {
    constructor(id){
        ...
        this.simulation = d3.forceSimulation()
            //.force("link", d3.forceLink().id(function(d) { return d.id; }).distance(function(d){return (150 - d.Confidence )}))
            .force("link", d3.forceLink().id(function(d) { return d.id; }).strength(function(d){return d.Confidence / 10000}).distance(50))
            //.force("link", d3.forceLink().id(function(d) { return d.id; }).strength(function(d){return d.Confidence / 100 / Math.min(count(d.source), count(d.target))}))
            //.force("link", d3.forceLink().id(function(d) { return d.id; }).strength(function(d){return  1 / Math.min(count(d.source), count(d.target))}))
        
            //.force("link", d3.forceLink().id(function(d) { return d.id; }))
            .force("charge", d3.forceManyBody().strength(this.manyForce))
            .force("radial", d3.forceRadial(this.width/3, this.width / 2, this.height / 2).radius(this.radRadius).strength(5.0))
...
class GraphPlot {
    ...
    runForceBundling(){
        let fbundling = d3.ForceEdgeBundling()
            .nodes(this.nodes)
            .edges(this.links.map(function(edge) {
                return {
                    source: simulation.nodes.indexOf(edge.source),
                    target: simulation.nodes.indexOf(edge.target),
                }
            }))
            .bundling_stiffness(0.3);
        this.forcedlinks = fbundling();
        Object.keys(this.links[0]).forEach(function(key) {
            for (let i=0; i < this.links.length; i++){
                this.forcedlinks[i][key] = this.links[i][key];
            }
        }, this);
    }
	enterSelection(){
		...
		//this.node_circles = this.node_circles
            ...
            .on("mouseover", this.toggle_selected)
            .on("mouseout", this.toggle_selected);
    }
	dragstarted(d) {
    ...
    toggle_selected(d, i){
        ...
        let linked_nodes;
        if (d.type == 'source')
            linked_nodes = new Set(Array.from(l.data(), link => link.target));
        else
            linked_nodes = new Set(Array.from(l.data(), link => link.source));

        let s_linked = d3.select('#nodes').selectAll('circle')
            .filter(function(d2, i2) {return linked_nodes.has(d2);})
        if (s.classed('selected')) {
            s.classed('selected',false);
            s_linked.classed('selected', false);
    ...
    positionLinks(links, plotObj){
        if (plotObj.link_type == 'line')
            ...
            links
                .attr('d', plotObj.bundleLine)
                //.attr('d', plotObj.linkAvoidCenter)
                .attr('fill', 'none');
    }
    linkAvoidCenter(d,i){
        let c = new Victor(100,100);
        let targetv = Victor.fromObject(d.source).subtract(c);
        let sourcev = Victor.fromObject(d.target).subtract(c);
	...
	showControls(){
		let data = [
				{x: 155, y: 5, action: 'hover', effect: 'highlight'},
				];
		let group = this.svg.append('g')
  			.attr('class', 'controls');
        group = group.selectAll("g")
...
parseDate = d3.timeParse("%Y%m%d%H%M");
...
function loadEventmentions(filename){
    return d3.csv(filename,
        function(d) {
            ...
            d.datetime = parseDate(d.MentionTimeDate)
            return d
    });
}
